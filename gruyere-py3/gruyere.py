...
import threading
import email
import subprocess #Kam importing subprocess for calling http.server to create a separate file server
import urllib
...
class GruyereRequestHandler(BaseHTTPRequestHandler):
  ...
  def _SendTextResponse(self, text, new_cookie_text=None):
    ...
    self._SendHtmlResponse('<pre>' + html.escape(text) + '</pre>',
                           new_cookie_text)
  ...
  def _SendTemplateResponse(self, filename, specials, params,
    ...
    f = None
    try:
      f = _Open(RESOURCE_PATH, filename, 'r')
      template = f.read()
    finally:
      if f: f.close()
  ...
  def _ParseCookie(self, cookie):
    ...
    try:
      ...
      # global cookie_secret
      if hashed != str(hash(str(cookie_secret) + cookie_data) & 0x7FFFFFF):
        return self.NULL_COOKIE
  ...
  def _DoUpload2(self, cookie, specials, params):
    ...
    try:
      ...
      if system_os == 'Windows':
        subprocess.call(['powershell', '-File', './file_server.sh'], cookie[COOKIE_UID]) #Kam calling the file server
      else:
        subprocess.call('sh', './file_server.sh', cookie[COOKIE_UID]) #Kam calling the file server
  ...
  def _ExtractFileFromRequest(self):
    ...
    Returns:
      (filename, file_data)
    """
    # Leggi la lunghezza del contenuto
    content_length = int(self.headers['Content-Length'])
        
    # Leggi il contenuto
    post_data = self.rfile.read(content_length)
        
    # Trova i confini del file
    ...
    for part in parts:
      if b'Content-Disposition: form-data;' in part:
        ...
        if b'filename="' in disposition:
          ...
          filename_start = disposition.find(b'filename="') + len(b'filename="')
          filename_end = disposition.find(b'"', filename_start)
          filename = disposition[filename_start:filename_end].decode('utf-8')
          ...
          # Qui puoi fare qualcosa con il file, ad esempio stamparlo
  ...
  def _SendRedirect(self, url, unique_id):
    ...
    self.send_header('X-XSS-Protection', '0')
    self.end_headers()
    html = '''<!DOCTYPE HTML PUBLIC '-//W3C//DTD HTML//EN'>
        <html><body>
